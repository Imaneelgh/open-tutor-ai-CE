# tests/test_chats.py
def _token(client):
    r = client.post("/auths/signup", json={"email": "chat@t.com", "name": "C", "password": "pass1234!"})
    return r.json()["token"]
...
def test_create_chat(client):
    token = _token(client)
    r = client.post("/api/v1/chats/new",
...
def test_get_chat_list(client):
    token = _token(client)
    client.post("/api/v1/chats/new", json={"chat": {"title": "A", "messages": []}}, headers=_auth(token))
...
def test_get_chat_by_id(client):
    token = _token(client)
    created = client.post("/api/v1/chats/new",
...
def test_update_chat(client):
    ...
    created = client.post("/api/v1/chats/new",
                          ...
                          headers=_auth(token)).json()
    r = client.post(f"/api/v1/chats/{created['id']}",
                    json={"chat": {"title": "New", "messages": [{"role": "user", "content": "hi"}]}},
                    headers=_auth(token))
    assert r.status_code == 200
...
def test_delete_chat(client):
    ...
    assert r.status_code == 200
    assert client.get(f"/api/v1/chats/{created['id']}", headers=_auth(token)).status_code == 404
...
def test_add_and_get_tag(client):
    ...
    created = client.post("/api/v1/chats/new",
                          ...
                          headers=_auth(token)).json()
    client.post(f"/api/v1/chats/{created['id']}/tags",
                json={"name": "work"}, headers=_auth(token))
...
def test_delete_all_chats(client):
    token = _token(client)
    client.post("/api/v1/chats/new", json={"chat": {"title": "A"}}, headers=_auth(token))
...
def test_cannot_access_other_users_chat(client):
    ...
    attacker = client.post("/auths/signup",
                           json={"email": "atk3@t.com", "name": "A", "password": "pass1234!"}).json()["token"]
    # Non-owner GET must 404 (not 403 — must not leak existence)
    assert client.get(f"/api/v1/chats/{created['id']}", headers=_auth(attacker)).status_code == 404
    # Non-owner UPDATE must also 404 (consistency with Fix 1)
    upd = client.post(f"/api/v1/chats/{created['id']}",
                      json={"chat": {"title": "Hacked", "messages": []}},
                      headers=_auth(attacker))
    assert upd.status_code == 404
    # Non-owner DELETE must 404
    assert client.delete(f"/api/v1/chats/{created['id']}", headers=_auth(attacker)).status_code == 404
