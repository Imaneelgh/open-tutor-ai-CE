def _admin_token(client):
    r = client.post("/auths/signup", json={
        "email": "admin@t.com", "name": "Admin", "password": "pass1234!"
    })
    return r.json()["token"]

def _user_token(client, email="user@t.com"):
    r = client.post("/auths/signup", json={
        "email": email, "name": "User", "password": "pass1234!"
    })
    return r.json()["token"]
...
def test_update_user_settings(client):
    token = _admin_token(client)
    r = client.post("/api/v1/users/user/settings/update",
                    json={"ui": {"theme": "dark"}},
                    headers=_auth(token))
    assert r.status_code == 200
...
def test_update_user_info(client):
    token = _admin_token(client)
    r = client.post("/api/v1/users/user/info/update",
                    json={"bio": "Teacher"},
                    headers=_auth(token))
    assert r.status_code == 200
...
def test_update_user_role(client):
    ...
    r = client.post("/api/v1/users/update/role",
...
def test_update_nonexistent_user_returns_404(client):
    token = _admin_token(client)
    r = client.post("/api/v1/users/does-not-exist/update",
                    json={"name": "Ghost"},
                    headers=_auth(token))
