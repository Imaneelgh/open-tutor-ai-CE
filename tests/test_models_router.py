...
def _token(client):
    r = client.post("/auths/signup", json={"email": "mdl@t.com", "name": "M", "password": "pass1234!"})
    return r.json()["token"]
...
def test_create_and_get_model(client):
    token = _token(client)
    r = client.post("/api/v1/models/create", json={
...
def test_toggle_model(client):
    token = _token(client)
    client.post("/api/v1/models/create",
...
def test_create_missing_id_returns_400(client):
    token = _token(client)
    r = client.post("/api/v1/models/create", json={"name": "NoId"}, headers=_auth(token))
    assert r.status_code == 400
...
def test_non_owner_cannot_delete_model(client):
    ...
    client.post("/api/v1/models/create",
                ...
                headers=_auth(owner))
    attacker = client.post("/auths/signup", json={"email": "atk@t.com", "name": "A", "password": "pass1234!"}).json()["token"]
