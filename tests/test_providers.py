def _admin(client):
    r = client.post("/auths/signup", json={"email": "prov@t.com", "name": "P", "password": "pass1234!"})
    return r.json()["token"]  # first signup = admin

def _nonadmin(client):
    client.post("/auths/signup", json={"email": "a0@t.com", "name": "A0", "password": "pass1234!"})  # admin
    r = client.post("/auths/signup", json={"email": "plain@t.com", "name": "P", "password": "pass1234!"})
    return r.json()["token"]
...
def test_openai_config_update_persists(client):
    token = _admin(client)
    client.post("/api/v1/providers/openai/config/update",
                json={"ENABLE_OPENAI_API": True,
...
def test_openai_urls_roundtrip(client):
    token = _admin(client)
    client.post("/api/v1/providers/openai/urls/update",
                json={"urls": ["https://x/v1", "https://y/v1"]}, headers=_auth(token))
...
def test_openai_keys_roundtrip_real_values(client):
    token = _admin(client)
    client.post("/api/v1/providers/openai/keys/update",
                json={"keys": ["sk-realsecret123"]}, headers=_auth(token))
...
def test_ollama_urls_roundtrip(client):
    token = _admin(client)
    client.post("/api/v1/providers/ollama/urls/update",
                json={"urls": ["http://h1:11434"]}, headers=_auth(token))
...
def test_openai_verify_unreachable(client):
    token = _admin(client)
    r = client.post("/api/v1/providers/openai/verify",
                    json={"url": "http://127.0.0.1:1/v1", "key": "sk-x"}, headers=_auth(token))
    assert r.status_code == 200
...
def test_ollama_verify_unreachable(client):
    token = _admin(client)
    r = client.post("/api/v1/providers/ollama/verify",
                    json={"url": "http://127.0.0.1:1", "key": ""}, headers=_auth(token))
    assert r.status_code == 200
    assert r.json()["status"] in ("unreachable", "error")
...
def test_config_persists_across_service_instances(client):
    ...
    token = _admin(client)
    client.post("/api/v1/providers/ollama/config/update",
                json={"ENABLE_OLLAMA_API": False, "OLLAMA_BASE_URLS": ["http://z:11434"], "OLLAMA_API_CONFIGS": {}},
                headers=_auth(token))
...
def test_openai_models_disabled(client):
    ...
    token = _admin(client)
    client.post("/api/v1/providers/openai/config/update",
                json={"ENABLE_OPENAI_API": False, "OPENAI_API_BASE_URLS": [], "OPENAI_API_KEYS": [], "OPENAI_API_CONFIGS": {}},
                headers=_auth(token))
...
def test_openai_chat_completions_disabled(client):
    ...
    client.post("/api/v1/providers/openai/config/update",
                json={"ENABLE_OPENAI_API": False, "OPENAI_API_BASE_URLS": [], "OPENAI_API_KEYS": [], "OPENAI_API_CONFIGS": {}},
                headers=_auth(token))
    r = client.post("/api/v1/providers/openai/chat/completions",
                    json={"model": "gpt-4", "messages": []}, headers=_auth(token))
    assert r.status_code == 503


def test_non_admin_can_access_chat_completions(client):
    """Proxy endpoints are non-admin — any authenticated user can call them."""
    nonadmin_token = _nonadmin(client)  # creates admin (a0@t.com) + plain user (plain@t.com)
    admin_token = client.post("/auths/signin", json={"email": "a0@t.com", "password": "pass1234!"}).json()["token"]
    client.post("/api/v1/providers/openai/config/update",
                json={"ENABLE_OPENAI_API": True,
                      "OPENAI_API_BASE_URLS": ["http://127.0.0.1:1/v1"],
                      "OPENAI_API_KEYS": [],
...
def test_openai_audio_speech_disabled(client):
    ...
    client.post("/api/v1/providers/openai/config/update",
                json={"ENABLE_OPENAI_API": False, "OPENAI_API_BASE_URLS": [], "OPENAI_API_KEYS": [], "OPENAI_API_CONFIGS": {}},
                headers=_auth(token))
    r = client.post("/api/v1/providers/openai/audio/speech",
...
def _setup_ollama(client, token, urls=None, enabled=True):
    client.post("/api/v1/providers/ollama/config/update",
                json={"ENABLE_OLLAMA_API": enabled,
...
def test_ollama_api_chat_disabled(client):
    token = _admin(client)
    _setup_ollama(client, token, enabled=False)
    r = client.post("/api/v1/providers/ollama/api/chat",
...
def test_ollama_embeddings_disabled(client):
    token = _admin(client)
    _setup_ollama(client, token, enabled=False)
    r = client.post("/api/v1/providers/ollama/api/embeddings",
...
def test_ollama_generate_disabled(client):
    token = _admin(client)
    _setup_ollama(client, token, enabled=False)
    r = client.post("/api/v1/providers/ollama/api/generate",
                    json={"model": "llama3", "prompt": "hi"},
                    headers=_auth(token))
    assert r.status_code == 503
...
def test_ollama_pull_admin_only(client):
    ...
    r = client.post("/api/v1/providers/ollama/api/pull",
                    json={"model": "llama3"}, headers=_auth(nonadmin))
    assert r.status_code == 403
...
def test_ollama_pull_disabled(client):
    ...
    _setup_ollama(client, token, enabled=False)
    r = client.post("/api/v1/providers/ollama/api/pull",
                    json={"model": "llama3"}, headers=_auth(token))
    assert r.status_code == 503

def test_ollama_create_admin_only(client):
    nonadmin = _nonadmin(client)
    r = client.post("/api/v1/providers/ollama/api/create",
...
def test_ollama_delete_admin_only(client):
    nonadmin = _nonadmin(client)
    r = client.request("DELETE", "/api/v1/providers/ollama/api/delete",
                       json={"model": "llama3"}, headers=_auth(nonadmin))
    assert r.status_code == 403
...
def test_ollama_download_invalid_host(client):
    token = _admin(client)
    _setup_ollama(client, token, urls=["http://127.0.0.1:11434"])
    r = client.post("/api/v1/providers/ollama/models/download",
                    json={"url": "https://evil.com/model.gguf"}, headers=_auth(token))
    assert r.status_code == 400

def test_ollama_download_admin_only(client):
    nonadmin = _nonadmin(client)
    r = client.post("/api/v1/providers/ollama/models/download",
                    json={"url": "https://huggingface.co/model.gguf"}, headers=_auth(nonadmin))
    assert r.status_code == 403
