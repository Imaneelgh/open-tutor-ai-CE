...
def _auth(client, email="kb@test.com"):
    client.post("/auths/signup", json={"email": email, "name": "KB", "password": "pass1234!"})
    r = client.post("/api/v1/auths/signin", json={"email": email, "password": "pass1234!"})
...
class TestKnowledgeApiV1:
    ...
    def test_get_by_id(self, client):
        ...
        kb = client.post("/api/v1/knowledge/create",
                         json={"name": "Science"},
                         headers=h).json()
    ...
    def test_update(self, client):
        ...
        kb = client.post("/api/v1/knowledge/create",
                         json={"name": "Old Name"},
                         headers=h).json()
        r = client.post(f"/api/v1/knowledge/{kb['id']}/update",
                        json={"name": "New Name"},
                        headers=h)
        assert r.status_code == 200
    ...
    def test_add_and_remove_file(self, client):
        ...
        kb = client.post("/api/v1/knowledge/create",
                         ...
                         headers=h).json()
        r = client.post(f"/api/v1/knowledge/{kb['id']}/file/add",
                        json={"file_id": "file-abc"},
                        headers=h)
        assert r.status_code == 200
        r2 = client.post(f"/api/v1/knowledge/{kb['id']}/file/remove",
                         json={"file_id": "file-abc"},
                         headers=h)
    ...
    def test_update_file(self, client):
        ...
        kb = client.post("/api/v1/knowledge/create",
                         json={"name": "File Update"},
                         headers=h).json()
        client.post(f"/api/v1/knowledge/{kb['id']}/file/add",
                    json={"file_id": "file-xyz"}, headers=h)
        r = client.post(f"/api/v1/knowledge/{kb['id']}/file/update",
