def _token(client):
    r = client.post("/auths/signup", json={"email": "cfg@t.com", "name": "C", "password": "pass1234!"})
    return r.json()["token"]
...
def _nonadmin_token(client):
    client.post("/auths/signup", json={"email": "admin0@t.com", "name": "A0", "password": "pass1234!"})  # first = admin
...
def test_set_and_get_roundtrip(client):
    ...
    client.post("/api/v1/configs/banners", json=[{"id": "b1", "content": "hi"}], headers=_auth(token))
