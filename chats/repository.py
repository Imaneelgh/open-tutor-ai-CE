...
class ChatRepository(BaseRepository[Chat]):
    ...
    def get_pinned(self, user_id: str) -> List[Chat]:
        return (self.session.query(Chat)
                .filter(Chat.user_id == user_id, Chat.pinned == True, Chat.archived == False)
    ...
    def add_tag(self, chat: Chat, tag_name: str) -> Chat:
        ...
        tags = meta.get("tags", [])
        if not any((t.get("name") == tag_name if isinstance(t, dict) else t == tag_name)
                   for t in tags):
    ...
    def archive_all(self, user_id: str) -> int:
        count = (self.session.query(Chat)
                 .filter(Chat.user_id == user_id, Chat.archived == False).count())
